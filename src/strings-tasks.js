...
function isPalindrome(str) {
  ...
  const first = chars.length - 1;
  const last = chars.length / 2;

  for (let i = 0; i < last; i += 1) {
    if (chars[i] !== chars[first - i]) {
      return false;
    }
  }
  return true;
...
function invertCase(str) {
  const lower = (char) => char.toLowerCase() === char;
  const invert = (char) =>
    lower(char) ? char.toUpperCase() : char.toLowerCase();
...
function encodeToRot13(str) {
  ...
  const last = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm!? ';
  let res = '';
  for (let i = 0; i < str.length; i += 1) {
